...
async function loadNewsDetail() {
  ...
  try {
    ...
    error.value = requestError instanceof Error ? requestError.message : '获取新闻详情失败'

    if (error.value.includes('404') || error.value.includes('新闻不存在')) {
      error.value = '新闻不存在'
    }
...
async function handleLikeComment(comment: CommentItem) {
  ...
  try {
    ...
    ElMessage.error(requestError instanceof Error ? requestError.message : '点赞评论失败')
  } finally {
    actionLoading.value = ''
  }
}
