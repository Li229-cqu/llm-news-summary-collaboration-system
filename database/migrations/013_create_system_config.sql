-- M10: system_config table for key-value configuration storage
-- Replaces the hardcoded mock get_system_config()

CREATE TABLE IF NOT EXISTS `system_config` (
  `id`            BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `config_key`    VARCHAR(100) NOT NULL COMMENT '配置键',
  `config_value`  TEXT COMMENT '配置值',
  `config_type`   VARCHAR(32) NOT NULL DEFAULT 'string' COMMENT '值类型: string/int/float/boolean/json',
  `description`   VARCHAR(255) DEFAULT '' COMMENT '配置说明',
  `editable`      TINYINT NOT NULL DEFAULT 1 COMMENT '是否可编辑: 1=可编辑, 0=只读',
...
INSERT INTO `system_config` (`config_key`, `config_value`, `config_type`, `description`, `editable`) VALUES
...
('ai.api_key', '', 'string', 'AI API 密钥(不明文返回前端)', 1),
...
('ai.sensitive_words', '[]', 'json', '敏感词列表(JSON数组)', 1),
