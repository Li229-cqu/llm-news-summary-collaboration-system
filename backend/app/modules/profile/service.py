...
# ── 完整报告按天内存缓存 ──
_weekly_report_full_cache: dict[str, Any] = {}

def _get_full_cache_key(user_id: int, range_days: int) -> str:
    """完整报告缓存键：user_id + range_days + 今天日期。"""
    return f"{user_id}:{range_days}:{date.today().isoformat()}"

def _get_cached_full_report(user_id: int, range_days: int) -> Any | None:
    """命中返回深拷贝，避免缓存被外部修改。"""
    key = _get_full_cache_key(user_id, range_days)
    data = _weekly_report_full_cache.get(key)
    if data is None:
        return None
    return _deepcopy_report(data)

def _save_cached_full_report(user_id: int, range_days: int, report: Any) -> None:
    """写入完整报告缓存（深拷贝后存储）。"""
    key = _get_full_cache_key(user_id, range_days)
    _weekly_report_full_cache[key] = _deepcopy_report(report)
...
def _deepcopy_report(obj: Any) -> Any:
    """深拷贝周报缓存对象，避免外部修改污染缓存。"""
...
def get_weekly_report(current_user: Optional[Any] = None, force_refresh: bool = False) -> WeeklyReportResponse:
    ...
    # 完整报告按天缓存：命中直接返回，跳过所有 SQL 和 AI 调用
    if not force_refresh:
        cached = _get_cached_full_report(user_id, 7)
        if cached is not None:
            return cached
    ...
    # 写入完整报告缓存（同一天复用）
    if not force_refresh:
        _save_cached_full_report(user_id, 7, result)
    return result
