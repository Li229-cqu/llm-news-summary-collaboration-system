...
| --- | --- | --- | --- | --- |
| GET | `/api/news/categories` | 否 | 获取新闻分类 | 数据库 `news_category`，失败时回退 `backend/app/mock/news.py` |
| GET | `/api/news` | 否 | 获取新闻列表 | 数据库 `news` + `news_category`，失败时回退 `backend/app/mock/news.py` |
| GET | `/api/news/hot` | 否 | 获取新闻热榜 Top10 | 数据库 `news` + `news_category`，失败时回退 `backend/app/mock/news.py` |
| GET | `/api/news/search` | 否 | 搜索新闻 | 数据库 `news` + `news_category`，失败时回退 `backend/app/mock/news.py` |
...
- `GET /api/news/categories`：首页左侧分类栏使用该接口读取数据库 `news_category`，失败时回退 mock。
- `GET /api/news?category_id=xxx`：点击左侧分类后，首页新闻列表按数据库 `news.category_id` 筛选。
- `GET /api/news/hot?limit=10`：首页右侧 Top10 使用数据库真实新闻统计排序。
